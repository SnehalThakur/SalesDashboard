...
import com.bizware.services.Pymongodb as db
...
async def create_sale(sale: schemas.SaleBase, db: Session = Depends(get_db),
...
async def update_sale(id: int, updated_sale: schemas.SaleBase, db: Session = Depends(get_db),
                      current_user: int = Depends(oauth2.get_current_user)):
    sale_query = db.query(models.Sale).filter(models.Sale.id == id)
